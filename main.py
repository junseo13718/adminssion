...
class UniversityRecommenderGUI:
    ...
    def save_results(self):
        if self.all_results is None:
            messagebox.showerror("저장", "먼저 대학 추천을 실행하세요")

            return

        file_path = filedialog.asksaveasfilename(title="추천 결과 Excel 저장",
                                                 defaultextension=".xlsx",
                                                 filetypes=[("Excel 파일", "*.xlsx")],
                                                 initialfile="대학추천결과.xlxs")
        ...
        try:
            ...
            with pd.ExcelWriter(file_path, engine="openpyxl") as writer:
                ...
                self.top_results.to_excel(writer, sheet_name="상위권추천", index=False)
